...
parser.add_argument('--prediction_output_dir', type=str, default="/home/tmp_user/code/output/render_output_disk2/prediction_v14",
...
def test():
    ...
    for test_idx in range(len(test_list)):
        ...
        with open(ann_path, 'r') as fh:
            ann = json.load(fh)
        if args.exclude_field_video and 'field_config' in ann and len(ann['field_config'])  >0:
            continue
        ...
        for what_if in range(-1, num_obj):
            ...
            if what_if==-1:
                ...
                objs_gt_tr = torch.stack(objs_gt, dim=1) 
                objs_pred = torch.stack(objs_pred, dim=1) 
                if args.visualize_flag:
                    sim_str_full = os.path.join(args.vis_dir, sim_str+'_'+str(what_if))
                    plot_video_trajectories(objs_gt_tr, loc_dim_st=0, save_id=sim_str_full+'_gt')
                    plot_video_trajectories(objs_pred[:, -future_frm_num : ], loc_dim_st=0, save_id=sim_str_full+'_query')
                    pdb.set_trace()
